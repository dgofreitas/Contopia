      ...
      if (isProvider) {
        ...
        if (TIMEOUT_ENABLED) {
          ...
          try {
            ...
            // --- SSE/Streaming diagnostics + chunk timeout ---
            const contentType = response.headers.get("content-type") || "";
            const isStream = contentType.includes("event-stream") || contentType.includes("stream");
            if (isStream && response.body) {
              ...
              log("debug", "HTTP_STREAM_START", { url, contentType, startTime });

              while (true) {
                try {
                  // Race entre read() e timeout por chunk
                  const chunkTimeoutPromise = new Promise((_, reject) => {
                    setTimeout(() => reject(new Error("STREAM_CHUNK_TIMEOUT")), STREAM_CHUNK_TIMEOUT_MS);
                  });
                  ...
                  if (done) {
                    log("debug", "HTTP_STREAM_END", { url, chunkCount, totalBytes, durationMs: Date.now() - startTime });
                    break;
                  }
                  chunkCount++;
                  totalBytes += value.length;
                  const now = Date.now();
                  const elapsedSinceLast = now - lastChunkTime;
                  lastChunkTime = now;
                  log("debug", "HTTP_STREAM_CHUNK", { url, chunkIndex: chunkCount, bytes: value.length, elapsedSinceLastMs: elapsedSinceLast, totalBytes });
                  chunks.push(value);
                } catch (e: any) {
                  if (e.message === "STREAM_CHUNK_TIMEOUT") {
                    log("error", "HTTP_STREAM_CHUNK_TIMEOUT", { url, totalBytes, waitTimeMs: STREAM_CHUNK_TIMEOUT_MS, lastChunkAtMs: lastChunkTime - startTime });
                    try { await reader.cancel(); } catch {}
                    throw new Error(`[StreamChunkTimeout] Provedor não enviou dados em ${STREAM_CHUNK_TIMEOUT_MS}ms. URL: ${url}`);
                  }
                  log("debug", "HTTP_STREAM_ERROR", { url, error: e.message });
                  break;
                }
              }

              // Reconstruct response for the engine
